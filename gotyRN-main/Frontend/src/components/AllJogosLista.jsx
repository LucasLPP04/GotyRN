...
const AllJogosLista = () => {
  ...
  const handleExcluirJogo = async (jogoId) => {
    try {
      const response = await fetch(`http://localhost:3000/excluirJogo/${jogoId}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok) {
        ...
        console.error('Erro ao excluir o jogo:', response.statusText);
      }
    } catch (error) {
  ...
  return (
    <div className="containerCrud">
      {jogos.map((item) => (
        <div key={item.JogosID} className="jogoCrud">
